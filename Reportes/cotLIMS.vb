...
Public Class cotLIMS
    Private Sub cmdBuscar_Click(sender As Object, e As EventArgs) Handles cmdBuscar.Click
        ...
        pago, entrega, modalidad, obser, calmetho, services As String
        Dim cve As Integer
        Dim R As String
        'Dim name, mfr, model, service, price, subt As String
        R = "Select [Cotizaciones].NumCot,FechaDesde,FechaHasta, [Prospectos]. Nombre + ' ' + Apellidos AS nom,Puesto, Telefono, Ext, Correo,
                [Empresas].Compania,DomicilioConsig,PartidaNo, Cantidad,[DetalleCotizaciones].[EquipId],
				[SetUpEquipment].[EquipmentName],[Mfr],[Model],[ServiceDescription],[SetupEquipmentServiceMapping].[Price],
				[Price] * Cantidad AS sub,[idUsuarioAdministrador], [Usuarios].[Nombre], [LugarCondicion].[Descripcon] AS lugar, [MonedaCondicion].[Descripcion] AS moneda, 
				...
				[Cotizaciones].[Observaciones],[CalibrationMethod], [ServiceDescription]
        ...
        'End If
        'If ((lectorMetasCotizador(13) Is DBNull.Value) OrElse (lectorMetasCotizador(13) Is Nothing)) Then
        '    name = " - "
        'Else
        '    name = lectorMetasCotizador(13)
        'End If
        'If ((lectorMetasCotizador(14) Is DBNull.Value) OrElse (lectorMetasCotizador(14) Is Nothing)) Then
        '    mfr = " - "
        'Else
        '    mfr = lectorMetasCotizador(14)
        'End If
        'If ((lectorMetasCotizador(15) Is DBNull.Value) OrElse (lectorMetasCotizador(15) Is Nothing)) Then
        '    model = " - "
        'Else
        '    model = lectorMetasCotizador(15)
        'End If
        'If ((lectorMetasCotizador(16) Is DBNull.Value) OrElse (lectorMetasCotizador(16) Is Nothing)) Then
        '    service = " - "
        'Else
        '    service = lectorMetasCotizador(16)
        'End If
        'If ((lectorMetasCotizador(17) Is DBNull.Value) OrElse (lectorMetasCotizador(17) Is Nothing)) Then
        '    price = " - "
        'Else
        '    price = lectorMetasCotizador(17)
        'End If
        'If ((lectorMetasCotizador(18) Is DBNull.Value) OrElse (lectorMetasCotizador(18) Is Nothing)) Then
        '    subt = " - "
        'Else
        '    subt = lectorMetasCotizador(18)
        'End If
        If ((lectorMetasCotizador(19) Is DBNull.Value) OrElse (lectorMetasCotizador(19) Is Nothing)) Then
            cve = " - "
        Else
            cve = lectorMetasCotizador(19)
            Select Case cve
                Case 1
        ...
        FrmReportes.ReportViewer1.LocalReport.ReportPath = "C:\Users\Software TI\Documents\GitHub\MetasCRM\Reportes\cotLIMSProspectos.rdlc"
        FrmReportes.ReportViewer1.LocalReport.SetParameters(New ReportParameter() {p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12,
